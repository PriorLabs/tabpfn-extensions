"""Tests for the image extension: `ImageTransformer` and `TabPFNWithImages`.

A stub replaces the vision encoder so no test downloads weights; one slow test runs
the real encoder when the optional dependencies and the Hub license are in place.
"""
...
def _estimator_data(*, regression: bool) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    n = SMALL_TEST_SIZE if FAST_TEST_MODE else DEFAULT_TEST_SIZE
...
class TestDeclaration:
    ...
    def test__no_declared_columns__is_identity_and_calls_no_encoder(
        ...
        assert list(transformer.get_feature_names_out()) == ["num", "photo"]
    ...
    def test__negative_or_non_integer_index__is_refused(self) -> None:
        with pytest.raises(ValueError, match="non-negative"):
            ImageTransformer([-1]).fit(_frame())
        with pytest.raises(ValueError, match="integers"):
            ImageTransformer(["photo"]).fit(_frame())

    def test__n_components_below_one__is_refused(self) -> None:
        with pytest.raises(ValueError, match="n_components"):
            _expander(n_components=0).fit(_frame())

    def test__array_input__is_refused(self) -> None:
        with pytest.raises(TypeError, match="DataFrame"):
            _expander().fit(np.zeros((3, 2)))
        with pytest.raises(TypeError, match="DataFrame"):
            ImageTransformer().fit(np.zeros((3, 2)))
...
class TestExpansion:
    ...
    def test__missing_cells__are_refused_naming_rows(self) -> None:
        ...
        with pytest.raises(ValueError, match="no image in rows 3, 5"):
    ...
    def test__non_base64_strings__are_refused_naming_rows(self) -> None:
        ...
        with pytest.raises(ValueError, match=r"rows 2: not base64"):
            _expander().fit(_frame(cells=cells))
    ...
    def test__two_declared_columns__are_both_expanded_in_order(self) -> None:
        ...
        assert transformer.expanded_indices == [0, 2]
        assert list(out.columns) == [
            "num",
...
class TestTransform:
    ...
    def test__missing_cell__is_refused_naming_the_row(self) -> None:
        ...
        with pytest.raises(ValueError, match="no image in rows 4"):
    ...
    def test__device_and_batch_size__are_handed_to_the_encoder(
        ...
        assert seen == [(torch.device("cpu"), 7)] * 2
...
class TestResolveDevice:
    def test__specs__resolve_to_torch_devices(self) -> None:
        ...
        assert resolve_device(torch.device("cpu", 0)) == torch.device("cpu", 0)
        assert resolve_device(["cpu", "cuda"]) == torch.device("cpu")
...
class TestDecoding:
    ...
    def test__open_images__converts_every_mode_to_rgb_and_resizes_to_a_square(
        self,
    ) -> None:
        payloads = [
            _png_bytes(),
        ...
        assert [image.size for image in images] == [(224, 224)] * 4

    def test__pixel_values__scales_and_normalises_per_channel(self) -> None:
        images = images_module._open_images([_png_bytes(colour=(255, 0, 128))])

        pixels = images_module._pixel_values(images)

        assert pixels.shape == (1, 3, 224, 224)
        assert pixels.dtype == torch.float32
        expected = (np.array([1.0, 0.0, 128 / 255]) - images_module.IMAGE_MEAN) / (
            images_module.IMAGE_STD
        )
        np.testing.assert_allclose(
            pixels[0, :, 100, 100].numpy(), expected, rtol=0, atol=1e-5
        )
...
class TestEncoderLoading:
    ...
    def test__missing_optional_dependency__names_the_extra(
        ...
        monkeypatch.setattr(images_module, "_ENCODER", None)

        with pytest.raises(ImportError, match=r"tabpfn-extensions\[image\]"):
            images_module._get_image_encoder()
    ...
    def test__gated_repo__is_reported_with_license_instructions(
        ...
        monkeypatch.setattr(
            images_module, "_import_transformers", lambda: GatedAutoModel
        )
        monkeypatch.setattr(images_module, "_ENCODER", None)
    ...
    def test__encoder__is_loaded_once_per_process(
        ...
        monkeypatch.setattr(images_module, "_import_transformers", lambda: FakeAuto)
        monkeypatch.setattr(images_module, "_ENCODER", None)

        first = images_module._get_image_encoder()
        again = images_module._get_image_encoder()

        assert again is first
        assert loaded == [IMAGE_ENCODER_MODEL]
...
class TestTabPFNWithImages:
    ...
    def test__classifier__fits_on_the_expanded_frame_and_predicts(
        ...
        n_expanded = 1 + min(N_COMPONENTS, len(X))
        assert len(model.image_transformer_.get_feature_names_out()) == n_expanded
    ...
    @pytest.mark.client_compatible
    @pytest.mark.local_compatible
    def test__no_declared_columns__delegates_as_is(
        self, tabpfn_classifier: Any, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(images_module, "encode_image_bytes", _never_called)
        X, y = _estimator_data(regression=False)
        ...
        model = TabPFNWithImages(tabpfn_classifier, image_features_indices=None)
        model.fit(X, y)

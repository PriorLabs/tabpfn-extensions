...
X_test = np.linspace(0.0, 10.0, 240, dtype=np.float32).reshape(-1, 1)


# ─────────────────────────── fit & full prediction ──────────────────────────

reg = TabPFNRegressor(n_estimators=4, random_state=0)
...
print(f"Predicted distribution: {logits.shape[1]} bars over {len(X_test)} test points")
...
mode_manual = criterion.mode(logits).numpy()
...
slice_indices = [int(np.argmin(np.abs(X_test[:, 0] - t))) for t in slice_targets]
...
y_widths_c = y_edges[1:] - y_edges[:-1]

...
`TabPFNWithImages` lets a TabPFN estimator take pictures alongside tabular fields. Each cell of a declared image column holds one image, as a base64 string (a `data:image/...;base64,` prefix is fine) or as the image file's bytes. At `fit` the column is replaced by up to 30 numeric features: the CLS embedding of a frozen [DINOv3 ViT-S/16](https://huggingface.co/facebook/dinov3-vits16-pretrain-lvd1689m), standardised and reduced by a PCA fit on the training rows. At `predict` the same encoder and PCA are applied. The wrapped estimator only ever sees numbers, so the local `tabpfn` package and the TabPFN client both work. `ImageTransformer` is the same step as a standalone scikit-learn transformer, for pipelines of your own.

...
class FinetunedTabPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    **kwargs : dict
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "FinetunedTabPFNClassifier":
        ...
        self.classifier_ = TabPFNClassifier(**classifier_config, fit_mode="batched", differentiable_input=False)
        # Required to access model parameters for the optimizer
        self.classifier_._initialize_model_variables()
        ...
        training_datasets = self.classifier_.get_preprocessed_datasets(
        ...
        optimizer = Adam(self.classifier_.model_.parameters(), lr=self.learning_rate)  # type: ignore
        ...
        for epoch in range(self.epochs):
            ...
            for (
                ...
                self.classifier_.fit_from_preprocessed(
                    X_context_batch, y_context_batch, cat_ixs, confs
                )

                # Get predictions (logits) on the fine-tuning set
                predictions = self.classifier_.forward(X_query_batch, return_logits=True)
            ...
            roc_auc, log_loss_score = evaluate_model(self.classifier_, eval_config, X_train, y_train, X_val, y_val)
            ...
            if self.early_stopping and not np.isnan(roc_auc):
                if roc_auc > best_roc_auc + self.min_delta:
                    ...
                    best_model_state = {key: value.clone() for key, value in self.classifier_.model_.state_dict().items()}  # type: ignore
                else:
                    patience_counter += 1
                ...
                if patience_counter >= self.patience:
                    ...
                    if best_model_state is not None:
                        self.classifier_.model_.load_state_dict(best_model_state)  # type: ignore
                    break
    ...
    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        ...
        # For prediction, TabPFN needs to see the full training data as context
        # Set fit_mode to standard mode for prediction to avoid the warning
        self.classifier_.fit_mode = "fit_preprocessors"
        self.classifier_.fit(self.X_, self.y_)

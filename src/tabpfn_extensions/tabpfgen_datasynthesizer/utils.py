...
def validate_tabpfn_data(
    ...
    if (
        len(unique_classes) <= 20 and len(unique_classes) > 0
    ):  # Assume classification if few unique values
        ...
        if len(counts) > 0:
            ...
            if min_count < 2:
                warnings_list.append(
                    "Some classes have very few samples - consider class balancing"
                )
            ...
            if imbalance_ratio > 5:
                warnings_list.append(
                    f"Dataset is imbalanced (ratio: {imbalance_ratio:.1f}:1) - consider using balance_dataset()"
                )
...
def analyze_class_distribution(
    y: np.ndarray | pd.Series, title: str = "Class Distribution"
) -> dict[str, Any]:
...
def calculate_synthetic_quality_metrics(
    ...
    try:
        ...
        warnings.warn(f"Could not calculate distribution metrics: {e}", stacklevel=2)
    ...
    if y_original is not None and y_synthetic is not None:
        try:
            ...
            js_div = 0.5 * np.sum(
                prop_orig_full * np.log(prop_orig_full / m)
